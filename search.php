...
if (isset($_GET['search'])) {
    ...
    if($keyword == ''){
        $sql = "SELECT * FROM sanpham WHERE name LIKE 'sdsaddw22' OR typee LIKE 'ádasđa22'";
        $laydudieusanpham = $conn->query($sql);
    }else{
        $sql = "SELECT * FROM sanpham WHERE name LIKE '%".$keyword."%' OR typee LIKE '%".$keyword."%'";
                    ...
                    while ($row = $laydudieusanpham->fetch_assoc()) {
                        ...
                        <div class="button">
                            <form method="post" action="giohang.php">
                                    ...
                                    <button class="button-89" type="submit" name="buynow">Buy Now</button>
                            </form>
                            <form method="post" action="giohang.php">
                                    <input type="hidden" name="product_id" value="'. $row["idsanpham"] .'">
                                    <input type="hidden" name="product_name" value="'. $row["name"] .'">
                                    ...
                                    <button class="button-89" onclick="addToCart()" type="submit" name="addToCart">Add to cart</button>
                            </form>
